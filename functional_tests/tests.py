...
class NewVisitorTest(StaticLiveServerTestCase):
    ...
    def test_can_start_a_list_and_retrieve_it_later(self):
        ...
        self.check_for_row_in_list_table('1: Buy notebook')
        self.check_for_row_in_list_table('2: Use notebook to write a story')        

        # Now a new user, Francis, comes along to the site.

        ## We use a new browser session to make sure that no information
        ## of Colleen's is coming through from cookies etc  
        self.browser.quit()
        self.browser = webdriver.Firefox()

        # Francis visits the home page.  There is no sign of Colleen's
        # list
        self.browser.get(self.server_url)
        page_text = self.browser.find_element_by_tag_name('body').text
        self.assertNotIn('Buy notebook', page_text)
        self.assertNotIn('Use notebook to write a story', page_text)

        # Francis starts a new list by entering a new item. He
        # is less interesting than Colleen...
        inputbox = self.browser.find_element_by_id('id_new_item')
        ...
        self.assertNotEqual(francis_list_url, colleen_list_url)

        # Again, there is no trace of Colleen's list
        page_text = self.browser.find_element_by_tag_name('body').text
        self.assertNotIn('Buy notebook', page_text)
